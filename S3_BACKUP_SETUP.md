...
Добавь строку:
```
0 3 * * * /var/www/country-house/scripts/backup-to-s3.sh >> /var/log/country-house-backup.log 2>&1
```
...
│   └── ...
└── tashi-ani/              # Бэкапы tashi-ani (если нужно)
    └── ...
```
...
aws s3 ls s3://copybases/country-house/ \
    --endpoint-url https://s3.regru.cloud \
    --region ru-1

# Скачать бэкап вручную
aws s3 cp s3://copybases/country-house/db-20251219_120000.sqlite ./backup.sqlite \
    --endpoint-url https://s3.regru.cloud \
    --region ru-1

# Загрузить файл в S3
aws s3 cp ./my-file.sqlite s3://copybases/country-house/ \
    --endpoint-url https://s3.regru.cloud \
    --region ru-1

# Удалить файл из S3
aws s3 rm s3://copybases/country-house/old-backup.sqlite \
    --endpoint-url https://s3.regru.cloud \
    --region ru-1
```
